...
let Accessory, Service, Characteristic, uuid, Formats, Perms;
...
module.exports = function(exportedTypes) {
    if (exportedTypes && !Accessory) {
        ...
        // HAP v2 moved Formats/Perms off Characteristic to the hap namespace top level.
        Formats = (exportedTypes.hap && exportedTypes.hap.Formats) || Characteristic.Formats;
        Perms = (exportedTypes.hap && exportedTypes.hap.Perms) || Characteristic.Perms;
...
function NestDeviceAccessory(conn, log, device, structure, platform) {
    ...
    this.addService = this.accessory.addService.bind(this.accessory);
    this.getService = this.accessory.getService.bind(this.accessory);
    this.setPrimaryService = this.accessory.setPrimaryService
        ? this.accessory.setPrimaryService.bind(this.accessory)
        : function () {};
...
NestDeviceAccessory.prototype.bindCharacteristic = function (service, characteristic, desc, getFunc, setFunc, format) {
    ...
    this.boundCharacteristics.push([service, characteristic, getFunc.bind(this)]);
};
...
NestDeviceAccessory.prototype.updateData = function (device, structure) {
    ...
    this.boundCharacteristics.map(function (c) {
        // HAP v2 removed Characteristic.getValue(); push the freshly computed
        // value via updateValue() (still fires the 'change' event).
        c[0].getCharacteristic(c[1]).updateValue(c[2]());
    });
};
